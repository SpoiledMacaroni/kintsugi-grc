...
def generate_category_a_compliant_encrypted(file_path: Path, password: str, record_count: int, key_mgr: KeyManager, root_dir: Path):
    """
...
def generate_category_b_false_positives(target_dir: Path, root_dir: Path):
    """
...
def generate_category_c_false_negatives(target_dir: Path, password: str, key_mgr: KeyManager, root_dir: Path):
    """
...
def generate_category_d_compliance_violations(etc_dir: Path, audit_dir: Path, billing_dir: Path, root_dir: Path):
    """
...
def generate_insecure_aes_ecb_ledger(file_path: Path, password: str, key_mgr: KeyManager, root_dir: Path):
...
def generate_zip_bomb(file_path: Path, root_dir: Path):
...
# -----------------------------------------------------------------------------
def build_healthcare_environment(base_dir: Path, key_mgr: KeyManager):
    ...
    generate_category_a_compliant_encrypted(
        ...
        1500, key_mgr, env_root
    )
    generate_category_c_false_negatives(env_root / "patient_records", "HealthCareSecurePass2026!", key_mgr, env_root)
    ...
    generate_category_d_compliance_violations(etc_dir, audit_dir, billing_dir, env_root)
    generate_active_directory_exports(etc_dir, key_mgr, env_root)

def build_merchant_environment(base_dir: Path, key_mgr: KeyManager):
    ...
    generate_category_b_false_positives(env_root / "inventory_control", env_root)
    ...
    etc_dir = env_root / "etc"
    audit_dir = env_root / "var" / "log" / "audit"
    generate_category_d_compliance_violations(etc_dir, audit_dir, pos_dir, env_root)
    generate_active_directory_exports(etc_dir, key_mgr, env_root)

def build_finance_environment(base_dir: Path, key_mgr: KeyManager):
    ...
    generate_category_c_false_negatives(env_root / "treasury_ops", "FinanceWireTransferSecret!", key_mgr, env_root)

    # Backups & Zip Bomb safety test
    backups_dir = env_root / "system_backups"
    generate_category_b_false_positives(backups_dir, env_root)
    generate_zip_bomb(backups_dir / "decompression_safety_check.zip", env_root)
    ...
    generate_category_d_compliance_violations(etc_dir, audit_dir, core_ledger_dir, env_root)
    generate_active_directory_exports(etc_dir, key_mgr, env_root)

def build_banking_environment(base_dir: Path, key_mgr: KeyManager):
    ...
    generate_category_c_false_negatives(env_root / "treasury_ops", "BankingSecretPass2026!", key_mgr, env_root)
...
def main():
    ...
    if "healthcare" in industries:
        build_healthcare_environment(output_base, key_mgr)
    if "merchant" in industries:
        build_merchant_environment(output_base, key_mgr)
    if "finance" in industries:
        build_finance_environment(output_base, key_mgr)
    if "banking" in industries:
        build_banking_environment(output_base, key_mgr)

    key_mgr.save()
    logger.info("Successfully completed synthetic environment generation.")
